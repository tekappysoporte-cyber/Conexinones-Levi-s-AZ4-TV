...
if not st.session_state.autenticado:
    ...
    # Sincronizamos con st.session_state para que no se pierdan al recargar
    ...
    with st.expander(
        ...
        with tab1:
            ...
            if st.button("Guardar Nueva Conexión", key="btn_add"):
                ...
                if t_val and c_val and box_val:
                    nuevo_registro = {
                        "Controlador": nuevo_controlador.strip() if nuevo_controlador else "",
                        "StoreNo": nuevo_store_no.strip() if nuevo_store_no else "",
                        "Tienda": t_val,
                        "Conexion": c_val,
                        "Caja": box_val,
                    }
                    ...
                    guardar_datos(datos)
                    # Actualizamos el session_state para mantener la sincronía
        ...
        with tab2:
            ...
            # Obtenemos registros de la tienda seleccionada actualmente si existe
            regs_mod_actuales = [
                d for d in datos if get_val(d, "tienda") == tienda_seleccionada
            ] if 'tienda_seleccionada' in locals() and tienda_seleccionada else []

            if regs_mod_actuales:
                opciones_mod = [
                    f"{get_val(r, 'caja')} - {get_val(r, 'conexion')}"
                    for r in regs_mod_actuales
                ]
                ...
                registro_actual = regs_mod_actuales[opciones_mod.index(seleccion_mod)]
        ...
        with tab3:
            ...
            tienda_del = st.selectbox(
                ...
                tiendas if 'tiendas' in locals() else [],
                index=None,
